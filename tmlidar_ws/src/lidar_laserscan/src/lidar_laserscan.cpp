...
lidarLaserScan::lidarLaserScan(ros::NodeHandle &nh, ros::NodeHandle &nh_priv) : nh_(nh), srv_(nh_priv), ring_count_(0)
{
  ros::SubscriberStatusCallback connect_cb = boost::bind(&lidarLaserScan::connectCb, this);
  pub_ = nh.advertise<sensor_msgs::LaserScan>("scan", 10, connect_cb, connect_cb);
  srv_.setCallback(boost::bind(&lidarLaserScan::reconfig, this, _1, _2));
}

void lidarLaserScan::connectCb()
{
  ...
  {
    sub_ = nh_.subscribe("lidar_points", 10, &lidarLaserScan::recvCallback, this);
  }
}

void lidarLaserScan::recvCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
  // Latch ring count
  if (!ring_count_)
  {
    ...
    if (!found)
    {
      ROS_ERROR("lidarLaserScan: Field 'ring' of type 'UINT16' not present in PointCloud2");
      return;
    }
    ...
    if (ring_count_)
    {
      ROS_INFO("lidarLaserScan: Latched ring count of %u", ring_count_);
    }
    else
    {
      ROS_ERROR("lidarLaserScan: Field 'ring' of type 'UINT16' not present in PointCloud2");
  ...
  ROS_INFO_ONCE("lidarLaserScan: Extracting ring %u", ring);
  ...
  if ((offset_x >= 0) && (offset_y >= 0) && (offset_r >= 0))
  {
    ...
    {
      ROS_WARN_ONCE("lidarLaserScan: PointCloud2 fields in unexpected order. Using slower generic method.");
  ...
  {
    ROS_ERROR("lidarLaserScan: PointCloud2 missing one or more required fields! (x,y,ring)");
  }
}

void lidarLaserScan::reconfig(lidarLaserScanConfig& config, uint32_t level)
{
  cfg_ = config;
}
