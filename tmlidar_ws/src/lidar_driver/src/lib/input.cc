...
namespace lidar_driver
{
  static const size_t packet_size =
      sizeof(lidar_msgs::lidarPacket().data);
  ...
  InputSocket::InputSocket(ros::NodeHandle private_nh, uint16_t port) : Input(private_nh, port)
  {
    ...
    ROS_INFO_STREAM("Version:1.0.8T");
    ...
    ROS_DEBUG("lidar socket fd is %d\n", sockfd_);
  }
  ...
  int InputSocket::getPacket(lidar_msgs::lidarPacket *pkt, const double time_offset)
  {
    ...
    while (true)
    {
      ...
      do
      {
        ...
        {
          ROS_WARN("UDP does not return data ,timeout");
          return -1;
        }
      ...
      ROS_DEBUG_STREAM("incomplete lidar packet read: " << nbytes << " bytes");
    }
                                                                              ...
                                                                              filename_(filename)
  {
    ...
    {
      ROS_FATAL("Error opening lidar socket dump file.");
      return;
    }
  ...
  /** @brief Get one lidar packet. */
  int InputPCAP::getPacket(lidar_msgs::lidarPacket *pkt, const double time_offset)
  {
    ...
    while (true)
    {
      ...
      {
        ROS_WARN("Error %d reading lidar packet: %s", res, pcap_geterr(pcap_));
        return -1;
      }
      ...
      ROS_DEBUG("replaying lidar dump file");
