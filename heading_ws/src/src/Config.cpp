...
void HeadingConfig::loadFromROS(ros::NodeHandle& nh) {
    ...
    nh.param<double>("ground_z_percentile", ground_z_percentile, ground_z_percentile);
    nh.param<int>("ground_inlier_min", ground_inlier_min, ground_inlier_min);
    nh.param<double>("ground_normal_threshold", ground_normal_threshold, ground_normal_threshold);
    ...
    nh.param<double>("wall_vertical_threshold", wall_vertical_threshold, wall_vertical_threshold);
    ...
    nh.param<double>("temporal_smoothing_tau", temporal_smoothing_tau, temporal_smoothing_tau);
    ...
    ROS_INFO("Forward axis: %s, vector: [%.2f, %.2f, %.2f]",
             forward_axis.c_str(), forward_vector.x(),
             forward_vector.y(), forward_vector.z());
    ROS_INFO("Sample points: front=%.2fm, back=%.2fm",
             sample_front_dist, sample_back_dist);
    ROS_INFO("Ground ROI radius: %.2fm, inlier min: %d",
             ground_roi_radius, ground_inlier_min);
    ROS_INFO("Wall height range: [%.2f, %.2f]m, inlier min: %d",
             wall_z_min, wall_z_max, wall_inlier_min);
    ROS_INFO("Voxel size: %.3fm, outlier: k=%d, std=%.2f",
             voxel_size, outlier_mean_k, outlier_std_mul);
    ROS_INFO("=======================================");
}
