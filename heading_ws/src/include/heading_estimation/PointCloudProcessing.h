...
class PointCloudProcessing {
   public:
    ...
    static pcl::PointCloud<pcl::PointXYZ>::Ptr passThroughZ(
        ...
        double z_max, bool negative = false);

    /**
     * @brief 基于分位数的高度滤波器
     * @param cloud 输入点云
     * @param z_percentile_min 最小高度分位数(0-1)
     * @param z_percentile_max 最大高度分位数(0-1)
     * @return 滤波后的点云
     */
    static pcl::PointCloud<pcl::PointXYZ>::Ptr passThroughZPercentile(
        const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
        double z_percentile_min, double z_percentile_max);
     ...
     * @brief RANSAC 平面拟合
     */
    static bool ransacPlane(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
                            Eigen::Vector3d& normal, double& d,
                            double& rms_error, int& inlier_count,
    ...
    static bool refinePlaneWeighted(
        const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
        Eigen::Vector3d& normal, double& d, const std::vector<double>& weights);

    /**
     * @brief 检查平面是否垂直
     */
    static bool isVerticalPlane(const Eigen::Vector3d& normal,
                                double z_threshold);

    /**
     * @brief 检查两个平面是否平行
     */
    static bool arePlanesParallel(const Eigen::Vector3d& n1,
