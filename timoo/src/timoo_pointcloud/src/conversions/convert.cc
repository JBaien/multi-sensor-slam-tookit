...
#include <timoo_pointcloud/organized_cloudXYZIR.h>
...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh,
    ...
    : data_(new timoo_rawdata::RawData(node, private_nh)),
      first_rcfg_call(true),
      diagnostics_(node, private_nh, node_name)
{
    // Get startup parameters
    private_nh.param<std::string>("fixed_frame", config_.fixed_frame, "timoo");
    private_nh.param<std::string>("target_Frame", config_.target_frame,
                                  "timoo");
    ...
    boost::optional<timoo_pointcloud::Calibration> calibration =
        data_->setup(private_nh);
    if (calibration) {
        ROS_DEBUG_STREAM("Calibration file loaded.");
        config_.num_lasers =
            static_cast<uint16_t>(calibration.get().num_lasers);
    } else {
    ...
    if (config_.organize_cloud) {
        container_ptr_ =
            boost::shared_ptr<OrganizedCloudXYZIR>(new OrganizedCloudXYZIR(
                config_.max_range, config_.min_range, config_.target_frame,
                config_.fixed_frame, config_.num_lasers,
    ...
    output_ = node.advertise<sensor_msgs::PointCloud2>("timoo_points", 10);

    srv_ = boost::make_shared<
        dynamic_reconfigure::Server<timoo_pointcloud::CloudNodeConfig> >(
        private_nh);
    dynamic_reconfigure::Server<timoo_pointcloud::CloudNodeConfig>::CallbackType
        f;
    f = boost::bind(&Convert::callback, this, _1, _2);
    srv_->setCallback(f);

    // subscribe to timooScan packets
    timoo_scan_ =
        node.subscribe("timoo_packets", 10, &Convert::processScan,
                       (Convert *)this, ros::TransportHints().tcpNoDelay(true));
    timoo_difop_ =
        node.subscribe("timoo_status", 10, &Convert::processDifop,
                       (Convert *)this, ros::TransportHints().tcpNoDelay(true));

    // Diagnostics
    diagnostics_.setHardwareID("timoo Convert");
    ...
    diag_topic_.reset(new TopicDiagnostic(
        "timoo_points", diagnostics_,
        FrequencyStatusParam(&diag_min_freq_, &diag_max_freq_, 0.1, 10),
        TimeStampStatusParam()));
}

void Convert::callback(timoo_pointcloud::CloudNodeConfig &config,
                       uint32_t level)
{
    ...
    if (first_rcfg_call || config.organize_cloud != config_.organize_cloud) {
        ...
        if (config_.organize_cloud) // TODO only on change
        {
            ...
            container_ptr_ =
                boost::shared_ptr<PointcloudXYZIR>(new PointcloudXYZIR(
                    config_.max_range, config_.min_range, config_.target_frame,
                    config_.fixed_frame, data_->scansPerPacket()));
        }
    }

    container_ptr_->configure(config_.max_range, config_.min_range,
                              config_.fixed_frame, config_.target_frame);
}

/** @brief Callback for raw scan messages. */
void Convert::processScan(const timoo_msgs::timooScan::ConstPtr &scanMsg)
{
...
void Convert::processDifop(const timoo_msgs::timooStatus &dpkt)
{
    data_->unpackDifop(dpkt);
}
