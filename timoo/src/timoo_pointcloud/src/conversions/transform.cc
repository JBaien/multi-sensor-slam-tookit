...
Transform::Transform(ros::NodeHandle node, ros::NodeHandle private_nh,
    ...
    : tf_prefix_(tf::getPrefixParam(private_nh)),
      data_(new timoo_rawdata::RawData),
      ...
      diagnostics_(node, private_nh, node_name)
{
    ...
    config_.target_frame = config_.fixed_frame = "timoo";
    ...
    if (config_.organize_cloud) {
        ...
        container_ptr = boost::shared_ptr<PointcloudXYZIR>(new PointcloudXYZIR(
            config_.max_range, config_.min_range, config_.target_frame,
            config_.fixed_frame, data_->scansPerPacket(), tf_ptr_));
    }
    ...
    // subscribe to timooScan packets using transform filter
    timoo_scan_.subscribe(node, "timoo_packets", 10);
    tf_filter_ptr_ =
        boost::shared_ptr<tf::MessageFilter<timoo_msgs::timooScan>>(
            new tf::MessageFilter<timoo_msgs::timooScan>(
                timoo_scan_, *tf_ptr_, config_.target_frame, 10));
    tf_filter_ptr_->registerCallback(
        boost::bind(&Transform::processScan, this, _1));
    private_nh.param<std::string>("fixed_frame", config_.fixed_frame, "odom");

    // Diagnostics
    diagnostics_.setHardwareID("timoo Transform");
...
void Transform::reconfigure_callback(
    timoo_pointcloud::TransformNodeConfig &config, uint32_t level)
{
    ...
    container_ptr->configure(config_.max_range, config_.min_range,
 ...
 */
void Transform::processScan(const timoo_msgs::timooScan::ConstPtr &scanMsg)
{
