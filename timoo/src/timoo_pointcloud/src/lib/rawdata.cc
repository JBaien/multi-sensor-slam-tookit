                            ...
                            double view_direction, double view_width)
{
    ...
    // config_.tmp_min_angle = fmod(fmod(config_.tmp_min_angle,2*M_PI) +
    // 2*M_PI,2*M_PI); config_.tmp_max_angle =
    ...
    // config_.min_angle = 100 * (2*M_PI - config_.tmp_min_angle) * 180 / M_PI +
    // 0.5; config_.max_angle = 100 * (2*M_PI - config_.tmp_max_angle) * 180 /
    // M_PI + 0.5; if (config_.min_angle == config_.max_angle)
    // {
...
bool RawData::buildTimings()
{
        ...
        for (size_t x = 0; x < timing_offsets.size(); ++x) {
            for (size_t y = 0; y < timing_offsets[x].size(); ++y) {
                ...
                timing_offsets[x][y] = (full_firing_cycle * dataBlockIndex) +
                                       (single_firing * (dataPointIndex + 1)) -
                                       single_packet_cycle;
            }
        }
    } else {
    ...
    if (timing_offsets.size()) {
        ...
        ROS_WARN(
            "NO TIMING OFFSETS CALCULATED. ARE YOU USING A SUPPORTED timoo "
            "SENSOR?");
    }
    return false;
}
...
boost::optional<timoo_pointcloud::Calibration> RawData::setup(
    ros::NodeHandle private_nh)
{
    ...
    //  std::cout<<config_.left_min_angle<<" "<<config_.left_max_angle<<"
    //  "<<config_.right_min_angle<<" "<<config_.right_max_angle<<std::endl;

    if (config_.left_min_angle < 0 || config_.left_min_angle > 360 ||
        config_.left_max_angle < 0 || config_.left_min_angle > 360 ||
    ...
    if (config_.right_min_angle < 0 || config_.right_min_angle > 360 ||
        config_.right_max_angle < 0 || config_.right_min_angle > 360 ||
 ...
 */
void RawData::unpack(const timoo_msgs::timooPacket &pkt,
                     DataContainerBase &data, const ros::Time &scan_start_time)
{
    ...
    float time_diff_start_to_this_packet =
        (pkt.stamp - scan_start_time).toSec();
    ...
    for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
        ...
        for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
            ...
            if ((block.rotation >= config_.left_min_angle &&
                ...
                float xx =
                    xy_distance * sin_rot_angle - horiz_offset * cos_rot_angle;
                // Calculate temporal Y, use absolute value
                float yy =
                    xy_distance * cos_rot_angle + horiz_offset * sin_rot_angle;
                if (xx < 0) xx = -xx;
                if (yy < 0) yy = -yy;

                // Get 2points calibration values,Linear interpolation to get
                // distance correction for X and Y, that means distance
                // correction use different value at different distance
                ...
                /// the expression wiht '-' is proved to be better than the one
                /// with '+'
...
{
    for (int i = 0; i < 16; i++) {
        timoo_pointcloud::LaserCorrection &corrections =
            calibration_.laser_corrections[i];
        // int AngleNoToChannelC16[16] =
        // {0,2,4,6,8,10,12,14,1,3,5,7,9,11,13,15};
        calibration_.laser_corrections[i].vert_correction =
            dpkt.vertical_angle_list[i] * M_PI / 180;

        calibration_.laser_corrections[i].cos_vert_correction =
            cosf(calibration_.laser_corrections[i].vert_correction);
        calibration_.laser_corrections[i].sin_vert_correction =
            sinf(calibration_.laser_corrections[i].vert_correction);
    }
}
...
bool RawData::compareLine(int a)
{
    ...
    for (int i = 0; i < hidelineNum.size(); i++) {
        if (a == hidelineNum[i]) return true;
    }
    return false;
}

void RawData::unpack_tm16(const timoo_msgs::timooPacket &pkt,
                          ...
                          const ros::Time &scan_start_time)
{
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
        ...
        for (int firing = 0, k = 0; firing < TM16_FIRINGS_PER_BLOCK; firing++) {
            for (int dsr = 0; dsr < TM16_SCANS_PER_FIRING;
                 dsr++, k += RAW_SCAN_SIZE) {
                timoo_pointcloud::LaserCorrection &corrections =
                ...
                azimuth_corrected_f =
                    azimuth + (azimuth_diff *
                               ((dsr * TM16_DSR_TOFFSET) +
                                (firing * TM16_FIRING_TOFFSET)) /
                               TM16_BLOCK_TDURATION);
                azimuth_corrected = ((int)round(azimuth_corrected_f)) % 36000;

                /*condition added to avoid calculating points which are not
                     in the interesting defined area (min_angle < area <
                   max_angle)*/
                if (!compareLine(corrections.laser_ring) ||
                    ...
                    // Calculate temporal X, use absolute value.
                    float xx = xy_distance * sin_rot_angle -
                               horiz_offset * cos_rot_angle;
                    // Calculate temporal Y, use absolute value
                    ...
                    // get distance correction for X and Y, that means distance
                    // correction use different value at different distance
                    float distance_corr_x = 0;
                    float distance_corr_y = 0;
                    if (corrections.two_pt_correction_available) {
                     ...
                     * model we used.
                     */
                    z = distance_y * sin_vert_angle +
                        vert_offset * cos_vert_angle;
                    ...
                    tm_point src_point(
                        ...
                        azimuth_corrected, distance, intensity, time);
                    tm_point dst_point;

                    if (filter_tm16(src_point, dst_point))
                        data.addPoint(dst_point.x, dst_point.y, dst_point.z,
                                      dst_point.ring, dst_point.azimuth,
                                      dst_point.distance, dst_point.intensity,
                                      dst_point.time);
                }
            }
            data.newLine();
...
bool RawData::filter_tm16(
    const tm_point &src_point,
    tm_point &dst_point) //去噪，转速、去噪参数，去噪阈值
                         ////去噪，转速、去噪参数，去噪阈值
{
    ...
    if (filter_pcloud[src_point.ring].size() == 3) {
        ...
        float delta1 = fabs(filter_pcloud[src_point.ring][0].distance -
                            filter_pcloud[src_point.ring][1].distance) *
                       200;
        float delta2 = fabs(filter_pcloud[src_point.ring][2].distance -
                            filter_pcloud[src_point.ring][1].distance) *
                       200;
        if (src_point.distance < 2.56) {
            if (delta1 < 8 * 0.1 * filter_threshold[0] &&
                delta2 < 8 * 0.1 * filter_threshold[0]) {
                // printf("1 : %lf  2 : %lf
            ...
            if (delta1 < 26 * 0.1 * filter_threshold[1] &&
                delta2 < 26 * 0.1 * filter_threshold[1]) {
                dst_point = filter_pcloud[src_point.ring][1];
                return_flag = true;
            }
        } else if (src_point.distance < 10.24) {
            if (delta1 < 48 * 0.1 * filter_threshold[2] &&
                delta2 < 48 * 0.1 * filter_threshold[2]) {
                dst_point = filter_pcloud[src_point.ring][1];
                return_flag = true;
            }
        } else {
            if (delta1 < 1024 * 0.1 * filter_threshold[3] &&
                delta2 < 1024 * 0.1 * filter_threshold[3]) {
                dst_point = filter_pcloud[src_point.ring][1];
                return_flag = true;
            }
        }
        filter_pcloud[src_point.ring].erase(
            filter_pcloud[src_point.ring].begin());
    }
